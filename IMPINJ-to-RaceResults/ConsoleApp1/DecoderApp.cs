...
namespace DecoderApp
{
	...
	public class DecoderApp
	{
		...
		public TcpClient? Rr12TcpClient { get => _rr12TcpClient; set => _rr12TcpClient = value; }
		...
		private void StartListeningForRr12()
		{
			...
			_ = Task.Run(async () =>
			{
				while (true)
				{
					_rr12TcpClient = new TcpClient(_settings.Rr12TcpClient, _settings.Rr12TcpPort);
					// _rr12TcpClient = await _rr12Listener.AcceptTcpClientAsync();
					Console.WriteLine("RR12 connected.");
					await HandleRr12Client(_rr12TcpClient);
				}
			});
		}
